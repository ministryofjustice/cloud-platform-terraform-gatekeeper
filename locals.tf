locals {
  ...
  constraint_map = {
    ...
    valid_hostname             = merge(yamldecode(file("${path.module}/resources/constraints/ingress_valid_hostname.yaml")), { "spec" : { "enforcementAction" : var.dryrun_map.hostname_length ? "dryrun" : "deny", "parameters" : { "validDomainNames" : "*.${var.cluster_domain_name},*.${var.integration_test_zone}" } } })
  }
}
